...

   {
     ...
     cars=[car1,car2,car3,car4];
      ...
      getRank()
      {
        ...
        var xyz=database.ref('Rank');
        xyz.on("value",function(data){rank=data.val()})
      }
     ...
     play()
     {
       ...
       if(allPlayers!==undefined)
       {
         ...
         text(allPlayers[plr].name + ":" + allPlayers[plr].distance,120,dpu)*/
           ...
           if(player.distance>3840 && check===0)
           {
             game1.update(2);
             rank++
             game1.updateRank(rank);
             var pi="Players/Player"+player.index;
             database.ref(pi).set({Rank:rank});
             check=1;
           }
           drawSprites();
